...
async def build_and_run_agent(agent_executor_uuid: str, connected_to: List[str]):
    ...
    request_handler = await get_request_handler(agent_executor, agent_card)
...
@t.command()
def main(agent_executor_uuid: str, connected_to: List[str]):
    asyncio.run(build_and_run_agent(agent_executor_uuid, connected_to))